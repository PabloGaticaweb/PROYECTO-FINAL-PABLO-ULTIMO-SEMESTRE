 ...
 *  Filtrar costos por criterios
 *  Consultar API para obtener detalles de costos
 *  Mostrar los resultados en tablas
 *  Exportar los datos obtenidos a Excel
 *  Navegación con Drawer (MUI)
 */
...
/**
 * Formatea valores numéricos a formato moneda USD
 * @param {number} value
 * @returns {string}
 */
...
export default function Costos() {
  ...
  /** Maneja apertura y cierre del menú móvil */
  ...
  /**
   * Envía filtros al servidor y obtiene resultados
   * @param {Event} e
   */
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/detalle-costos", {
        method: "POST",
  ...
  /**
   * Exporta los datos obtenidos a un archivo Excel
   */
  const exportarExcel = () => {
  ...
  /**
   * Renderiza tablas con los resultados
   */
  const renderDetalle = () => {
    if (!detalle || Object.keys(detalle).length === 0) {
